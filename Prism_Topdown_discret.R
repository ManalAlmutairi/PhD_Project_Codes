...
# Three types of top-down discretisation methods (CAIM, CACC, Ameva) are used 
# in this implementation of Prism  
...
# Three types of top-down discretisation algorithms (1=CAIM, 2=CACC, 3=Ameva)

Disc.method <- 1

# To display the name of the discretisation method in the final output title
if (Disc.method == 1 )   {myMethod <- "CAIM"
}else if (Disc.method == 2) {myMethod <- "CACC"
} else if ( Disc.method == 3) myMethod <- "Ameva"

print ( "Discretisation in progress ...")
# browser()

# Processing Step: Convert the numeric data attributes values into categorical values using chiMerge algorithm
numToCat <- disc.Topdown(toBeDiscretise, method= Disc.method)
 # browser()

# rejoin myData to include both: the discretised data and the categorical data
if (CategAttrExist) {

   allMyDataCateg <- cbind(onlyCategData, numToCat$Disc.data)
} else {
   allMyDataCateg <- numToCat$Disc.data
}
 ...
 # cat ('\n Current Instance :')
  ...
  #    cat (' \n the first matching rule occured in the previous list will be chosen \n')
      ...
      if  (validatePrediction == TRUE )  {
         ...
         cat ( 'Rule number: ', thisRuleIndx, ' used correctly ',usedRulesList$`Correctly used`[thisRuleIndx] , ' time(s) \n')
...
cat ('\n ############# PRISM Final Results using ',myMethod,' discretisation algorithm ############# \n \n ', '  Dataset: ', fileName, ' (', seedNo , ') \n'   )
