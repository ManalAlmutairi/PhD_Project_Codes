...
# The implementation of Original PRISM algorithm in R
   ...
   while (keepGoing)  {
      ...
      while ( f.classesOtherThanGivenClass (datasetD, i) == TRUE ) {    
         ...
         for (attr in predictAttr:allPredictAttr) {      # loop2   
            ...
            # here is the problem: no examples in dataset belong to the current class and the only examples left is from another class
            ...
            if ( is.numeric(currentAttr)) {
               ...
               # the following loop to discretise the current data vector using repeated cut point calculations (original Prism approach)
               ...
               for (attrVal in levels(as.factor(currentAttr))) {    # loop3
         ...
         #browser()
         rowToAdd <- ruleTermProbabilities [which(as.numeric(ruleTermProbabilities[,7]) == highestProbability) , ]
  ...
  # cat ( ' \n instance ', anExample, ': \n')
   ...
   for  (ruleNo in 1: rulesNumber) {     # for 2
      ...
      for ( aTerm in  1:aRuleNumTerms)  {    # for 3
         ...
         if  (numericExample ) {
            ...
            # the following else statment will be excuated when it is categorical attribute      
         } else if  ( currentInstance [[termAttributeName]] == termAttributeValue)  {
            matchedTerms <- matchedTerms + 1
   ...
   if ( numOfRuleCoveredInstance > 0 ) {    
      ...
      if (is.na (usedRulesList$Rules[1]))  {
         ...
         if (foundIt) {
            #cat  ( '\n Rule is already used before: ', usedRulesList$`Used Times`[ruleExistIndx], ' times')
            #browser()
      ...
      if  (validatePrediction == TRUE )  {
         ...
         # cat ( '\n rule number: ',ruleNo, ' is used correctly ',usedRulesList$`Correctly used`[thisRuleIndx] , 
      ...
      # cat ( ' \n \n There is no rule covered instance number: ', anExample, 'majority class will be considered \n' )
      # browser()
...
# When NA occur in confusion matrix, this means that true positive = 0 or false nagative = 0 ! ..
# I tried to replace NA with zero values before taking the averager values in case of multiclasses 
...
byRulesConfusionMat <- confusionMatrix ( classifiedInstancesByRules$`Predicted Class`,
                                         classifiedInstancesByRules$class, mode = "everything")
...
# Important Note: some of the following calculations are calculated using two different ways for a validation purposes!
...
tentitiveAccuracy <- truePositiveExamples / NumOfExmpCoveredByRule

byRulesClassificationCorrect <- classifiedInstancesByRules [ which (classifiedInstancesByRules$`Predicted Class` == classifiedInstancesByRules$class ), ] %>% nrow()
...
cat ('\n ############# PRISM Final Results using cutpoints calculations ############# \n \n ', '  Dataset: ', fileName, ' (', seedNo , ') \n'   )


cat ( '\n Train Data: ', nrow(trainData)  )
cat (' \n Test Data: ', totalClassifiedInstances)

cat (' \n Stopping Critiera is used?', (if_else(stoppingCriteriaUsed , "Yes" , "No")) , '|  (Minimum Rule Accuracy = ', minimumRuleAccuracy *100, '%) \n' )
cat ( '\n Limited Terms/Rule are used?', (if_else(limitedTermsUsed , "Yes" , "No" ))  , '| ', maxTerms + 1, ' Terms/Rule')
cat ( '\n *****************************************************************')

cat ( '\n number of Rules: ', length(rulesLibrary.R))
cat ( '\n average of terms/rule: ', totalNumOfRuleTerms / rulesNumber)

cat (' \n Total Number of examples covered by Rules =  ', NumOfExmpCoveredByRule )
cat (' \n Number of examples correctly covered by Rules (True Positive) = \n               ', truePositiveExamples ,
     ' (manually) ' , byRulesClassificationCorrect , ' (using the by_Rules_Classification_Table)')
cat (' \n Number of examples wrongly covered by Rules (False Positive) =', falsePositiveExamples, '\n')
...
cat ('\n \n  Abstaining Rate = ', (unClassifiedInstances / totalClassifiedInstances ) )
...
cat('\n \n Tentitive Accuray = ', tentitiveAccuracy, ' ( from incremental Counter)\n                     ',
