...
# The implementation of PRISM algorithm using ChiMerge discretisation in R
...
# clear the environment working place
...
# fileName <- "data/EEG_Eye_State.data"

# fileName <- "data/post-operative.data"

# fileName <- "data/Chess.data"        # mixed dataset
#
# fileName <- "data/letter_recognition.data"

# fileName <- "data/breast-cancer-wisconsin.data"
#
# fileName <- "data/skin_segmentation.data"
...
space_group <- c ( "data/iris.data", "data/seeds.data", "data/ecoli.data", "data/yeast.data", "data/page-blocks.data", "data/skin_segmentation.data" )
comma_group <- c ( "data/wine.data", "data/transfusion.data", "data/banknote.data", "data/glass.data", "data/HTRU_2.data" ,
                   "data/magic04.data" , "data/EEG_Eye_State.data", "data/letter_recognition.data", "data/post-operative.data", 
                   "data/Chess.data", "data/breast-cancer-wisconsin.data")
semicolon_group <- c ( "data/winequality-white.data",  "data/winequality-red.data" )
xlsx_group <-  (  "data/BreastTissue.xlsx" )


if (fileName %in% space_group) {
   myData <- read.table(fileName ,header = FALSE , stringsAsFactors = FALSE)
} else if ( fileName %in% comma_group) {
   myData <-  read.table (fileName , header = FALSE, sep = "," , stringsAsFactors = FALSE)
} else if ( fileName %in% xlsx_group) {
   myData <- read.xlsx( fileName, sheetNo ,  as.data.frame=TRUE, header=TRUE, stringsAsFactors = FALSE)
}  else if ( fileName %in% semicolon_group) {
...
if (fileName == "data/lenses.data")  {
    ...
    myData$`Bare Nuclei` <- as.numeric(myData$`Bare Nuclei`)
    
} else if (fileName == "data/glass.data") {
    names(myData) <- c ( "Id", "RI", "Na", "Mg", "Al","Si", "K", "Ca", "Ba", "Fe", "class")
    
} else if  (fileName == "data/HTRU_2.data") {
    names(myData) <- c("IP mean", "IP sd", "IP kurtosis", "IP Skewness", "DM-SNR mean", "DM-SNR sd", "DM-SNR kurtosis", "DM-SNR Skewness", "class")
    
} else if ( fileName == "data/fertility_Diagnosis.data") {
    names (myData) <- c ( "Season", "Age", "Childish diseases", "Accident", "Surgical intervention",
    "High fevers", "alcohol consumption", "Smoking", "sitting hours", "class")
    
} else if ( fileName == "data/haberman.data") {
    names (myData) <- c ( "Age", "operation year", "positive axillary nodes", "class")
    
} else if (fileName == "data/EEG_Eye_State.data" ) {
    names (myData) <- c ( "AF3", "F7", "F3", "FC5", "T7", "P7", "O1", "O2", "P8", "T8","FC6",
    "F4","F8", "AF4", "class" )
    
} else if (fileName == "data/letter_recognition.data") {
    names (myData) <- c ( "class", "x-box", "y-box", "width", "high ", "onpix", "x-bar", "y-bar", "x2bar",
    "y2bar", "xybar", "x2ybr", "xy2br", "x-ege", "xegvy", "y-ege", "yegvx"  )
    
} else if (fileName == "data/post-operative.data" ) {
    names (myData) <- c ("L-CORE", "L-SURF", "L-O2", "L-BP", "SURF-STBL", "CORE-STBL", "BP-STBL",
    "COMFORT", "class")
    myData$COMFORT <- as.numeric(myData$COMFORT )
} else if ( fileName == "data/Chess.data") {
    names(myData) <- c( "White King file", "White King rank", "White Rook file", "White Rook rank", "Black King file", "Black King rank", "class" )
    
}

# Remove the first column from the predictive attributes list in case of it contained class labels or Ids 
#if ( classCol == 1 | colnames(myData[1]) == "Id" | colnames(myData[1]) == "Sequence Name"  ) { 
columnsDiscarded <- FALSE
if ( colnames(myData[1]) == "Id" | colnames(myData[1]) == "Sequence Name" | colnames(myData[1]) == "code number"  ) { 
   discardedAttr <- 1
   columnsDiscarded <- TRUE
   predictAttr <-  1  
   
} else predictAttr <- 1   


# check if any coulmns have been discarded previously (ids / sequence number ...)
if (columnsDiscarded ) {
   myData <- myData [ , -discardedAttr]
}
...
# Total number of predictive attributes in the dataset.
allPredictAttr <- colNum - 1 
...
# Cleaning data :dealing with missing values (none class labels) if any
missingValues <- FALSE
if ( any (myData == "?" | is.na(myData)))  {
    missingValues <- TRUE
}

if (missingValues) {
    tempData <- replace(myData, myData == "?", NA)
    any(is.na(myData))
    myData <- tempData
...
# # browser()
# Normalise data set values in case of negative values because chiMerge function does not deal with negative values
if (any(myData [, -classCol] < 0)) {
    # browser()
    #Normalize Data
    for ( normaliseThis in 1 : colNum ) {
...
# The following statement creates the opposite of %in% function
`%notin%` <- Negate(`%in%`)

# This function ckecks for the empty variables ( integer(0))
is.integer0 <- function(x)
{
   is.integer(x) && length(x) == 0L
}

####  Pre-processing step: Dealing with numerical Attributes  ####
#----------------------------------------------------------
...
# # 1st  way to subset a new data frame contained the numerica attibutes only     
# dataContainsNumericAttr <-  myData[,names(myData) %in% names(numericCols)]
# 

# 2nd way: the easiest way to subset the numeric attributes from the myData  (better than above)
onlyNumericData <- myData[numericCols]

####  Pre-processing step: Dealing with Categorical Attributes  ####
#-------------------------------------------------------------

# extracting categorical attributes from the data
checkForCat <- !(sapply(myData [,] , is.numeric) )
categCols <- which(checkForCat)

# 1st  way to extract categorical attributes from the data
categCols <- which(names(myData[,-classCol]) %notin%  names(numericCols))

CategAttrExist <- FALSE
if (!(is.integer0 (categCols))) {
   onlyCategData <- myData [categCols]
   CategAttrExist <- TRUE
   }



#There is a problem: if the first column is the class label, chiMerge does not recoginse it as a class column <<<<<< SOLVED

# create a Data frame that contained only numeric attributes in order to be used in the discretsation methosds later
#toBeDiscretise <- cbind(onlyNumericData, class = myData$class) 

print ( "Discretisation in progress ...")

# browser()
# Pr-eprocessing step: convert the numeric data attributes values into categorical values using chiMerge algorithm
 # numToCat<- chiM (myData , alpha = 0.05)
 
 # myData.disc <- chiMerge(myData ,numericCols, alpha = 0.05, out = "num" )



# if ( classCol == 1  ) {
#    
#    myData.disc <- chiMerge(myData, 2:allPredictAttr, alpha = 0.05 , out = "num")   
# } else if ( classCol == colNum ) {
#    myData.disc <- chiMerge(myData, 1:allPredictAttr , alpha = 0.05 , out = "num")
# }
# browser()
# numToCat <- chi2(myData , alp = 0.1, del = 0.05 )
 # browser()

# rejoin myData to include both: the discretised data and the categorical data
# if (CategAttrExist) {
# 
#    allMyDataCateg <- cbind(onlyCategData, myData.disc)
#    # allMyDataCateg <- cbind(onlyCategData, numToCat$)
# } else {
#    allMyDataCateg <- myData.disc
# }


# testing the generated converted data frame by comparing the attributes names and the size of the data frame 
# with the original myData data frame

# ncol(allMyDataCateg) == ncol(myData)
# 
# names(allMyDataCateg) == names(myData)
      
# merge(as.data.frame( dataContainsCateg), as.data.frame( numToCat) , by=0, all=TRUE)

numToCat <- modChi2(myData , alp = 0.05)



# convert the numerical discretised data into factors to be treated as categorical variable in PRISM algorithm
# myData <- allMyDataCateg
...
# This datasets is already sampled (first 3133 used for training) and (final 1044 used for testing)
     ...
     # cat ('\n****************\n', 'Hello Class:  ', i, '*************** \n')
      ...
      while ( f.classesOtherThanGivenClass (datasetD, i) == TRUE ) {    
         ...
         for (attr in predictAttr:allPredictAttr) {      # loop2   
            ...
            datasetForCurrentClass <- datasetD [datasetD$class == i , ]
            currentAttr <- datasetForCurrentClass[,attr]
       ...
       # which line has the highest probablities
       row.names( ruleTermProbabilities[ which.max(as.numeric(ruleTermProbabilities[ ,7])) ,])
      ...
      #   print (dataCoveredByBestProbability)
         ...
         if (nrow (dataCoveredByBestProbability) == 0 | !any (dataCoveredByBestProbability$class == i) ) {
      ...
      if (!(is.na(listOfRuleTerms[1,1]))) {
         ...
         if (noMoreAtt | noInstCoveredRuleTerm | reachMaxTermsNum) { 
           ...
           # cat (' \n WORNING: DatasetD contains classes other than the target class. As there are no more attributes to spit on!
            ...
            # second: find the size of current datasetD for the target class i
            currentDatasetDSize <- nrow ( datasetD [ datasetD$class == i,])
            
            currentOriginalDataSize <- nrow (originalData [ originalData$class == i , ])
           ...
           #    cat ( '\n', covPos , ' instances out of ', nrow(datasetD), ' are positivly covered by the rule \n')
            ...
            for (eachRuleTerm in 1:nrow(listOfRuleTerms)) {
                aTermAttribute <- which (colnames(originalData) == listOfRuleTerms$Attribute[eachRuleTerm])
     ...
     #       #print (aRule[aTerm,])
      ...
      if (matchedTerms == aRuleNumTerms) {
        ...
        ruleCovIns <- ruleCovIns + 1
         ...
         rulescoveredInstance [[ruleCovIns]] <- aRule
  ...
  # print ( rulescoveredInstance)
   ...
   if ( numOfRuleCoveredInstance > 0 ) {    
      ...
      if  (validatePrediction == TRUE )  {
         ...
         cat ( '\n Rule number: ', thisRuleIndx, ' is used correctly ',usedRulesList$`Correctly used`[thisRuleIndx] , 
               ' time(s)  out of ', usedRulesList$`Used Times`[thisRuleIndx] ,'\n')
     ...
     # cat ( 'There is no rule covered instance number: ', anExample )
...
cat ('\n ############# PRISM Final Results using ChiMerge discretisation method ############# \n \n ', '  Dataset: ', fileName, ' (', seedNo , ') \n'   )
